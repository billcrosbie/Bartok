...
// An enum to handle all te possible scoring events
...
public class Prospector : MonoBehaviour {
	...
	void LayoutGame() {
		...
		foreach (SlotDef tSD in layout.slotDefs) {
						...
						// This replaces the previous parent: deck.deckAnchor, which appears as _Deck in the
						// Hierarchy when the scene is playing.
			...
			cp.SetSortingLayerName(tSD.layerName);	// Set the sorting layers
						...
						tableau.Add (cp); // Add this CardProspectore to the List<> tableau
				}
		...
		UpdateDrawPile ();
		Debug.Log ("Calling UpdateDrawPile");
	}
	...
	public void CardClicked(CardProspector cd) {
		...
		case CardState.target:
			Debug.Log ("The target has been clicked on");
			// Clicking the target card does nothing
			break;
		case CardState.drawpile:
			Debug.Log ("The drawpile has been clicked on");
	...
	void MoveToDiscard(CardProspector cd) {
		Debug.Log ("MoveToDiscard has been called");
	...
	void MoveToTarget(CardProspector cd) {
		Debug.Log ("MoveToTarget has been called");
	...
	void UpdateDrawPile() {
		Debug.Log ("UpdateDrawPile has been called");
		CardProspector cd;
	...
	void SetTableauFaces() {
		foreach (CardProspector cd in tableau) {
			...
			cd.faceUp = fup;	// Set the value of the card
		}
	}
	...
	void ScoreManager(ScoreEvent sEvt) {
		List<Vector3> fsPts;
		switch(sEvt) {
			// Same things need to happen whether it's a draw, a win, or a los
